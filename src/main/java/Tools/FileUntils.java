...
public class FileUntils {
     ...
     * return Map类型
     */
    public static void writeYaml(Object obj,String yamlpath)
    {
        try{
            Yaml yaml = new Yaml();
            yaml.dump(obj, new BufferedWriter(new FileWriter(yamlpath)));
        }catch (Exception e){
            System.out.println(String.format("写入%s异常! + \n + %s", yamlpath,e));
        }
    }
